...
def train(args_override):
    ...
    # no project
    if RANK == 0 and args.no_project:
        print("No Project! dataset will work in no project mode")
        ...
        if RANK == 0:
            ...
            if (epoch + 1) % args.save_epochs == 0:
                torch.save(
                    ...
                    os.path.join(args.ckpt_dir, "DSP_policy_epoch_{}_seed_{}.ckpt".format(epoch + 1, args.seed))
                )
                plot_history(train_history, epoch, args.ckpt_dir, args.seed,identifier="DSP")

    if RANK == 0:
        torch.save(
            policy.module.state_dict(),
            os.path.join(args.ckpt_dir, "DSP_policy_last.ckpt")
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_project', action = 'store_true', help = 'turn off projector and use workspace normalization')
